...
public class GameOver extends JFrame implements ActionListener {
	...
	public GameOver() {
		...
		ImageIcon gameOver = new ImageIcon("resources/images/gameOver.png");
		...
		URL url = this.getClass().getClassLoader().getResource("resources/audio/defeat.wav");
		music = new Audio(url);
			...
			if (Jeu.currentLevel == 3) { /*
				...
				for (int i = 0; i < Jeu.listeObstacles.size(); i++) {
					if (Jeu.listeObstacles.get(i) instanceof Boss)
						((Boss) Jeu.listeObstacles.get(i)).enemySpawnTimer.stop();
				}
			}
