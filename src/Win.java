...
public class Win extends JFrame implements ActionListener {
	...
	public Win() {
		...
		ImageIcon win = new ImageIcon("resources/images/bravo.png");
		...
		URL url = this.getClass().getClassLoader().getResource("resources/audio/win.wav");
		music = new Audio(url);
		music.jouer();
	...
	public JLabel displayScore(long finalTime) {
		...
		if (Jeu.getHighScore() > (int) finalTime) {
			...
			JLabel s = new JLabel("TU AS FAIS : " + minutes + "min" + seconds + "s. HIGHSCORE : " + h_minutes + "min"
					+ h_seconds + "s.", SwingConstants.CENTER);
			s.setLayout(null);
			s.setFont(new Font("Arial", Font.BOLD, 15));
			s.setBounds(0, 320, this.getSize().width, 150);
			s.setForeground(Color.white);
