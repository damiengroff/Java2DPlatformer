...
public class Player extends JFrame {
	...
	public Player(int x, int y) {
        ...
        tailleY = 70;
        munitions = 10;
        tempsInvincibilite = 1000;
        invincible = false;
        jump = false;
        
        try {
			for(int i = 0; i < images.length; i++) {
				String access = "./Images/player/p"+i+".png";
	...
	private int animate() {
		if (speed < 0 && isCollisionBas(Jeu.listeObstacles)) { //à gauche
			if (currentImage < 9 && currentImage >= 4) {
				if (currentImage == 8) {
					currentImage = 5;
					return 5;
				} else {
	...
	public void move(ArrayList<Obstacles> l) {
		...
		if (!isCollisionBas(l)) {
			posY += customCast(0.5*10*Math.pow(0.001*fallTimer, 2)
								+1.0125);
		}
		if (jump && fallTimer < 100 && !isCollisionHaut(l)) {
			posY -= customCast(Math.pow(-0.0003*fallTimer, 2)+3);
			
		} else if (jump && fallTimer > 100) {
			jump = false;
		}
